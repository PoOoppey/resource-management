    ...
    if overview_df is not None and not overview_df.empty:
        ...
        if selected_rows:
            ...
            if 0 <= row_position < len(overview_df.index):
                ...
                st.session_state["allocation_selected_employee"] = selected_employee_uuid

    if not selected_employee_uuid:
        previous_selection = st.session_state.get("allocation_selected_employee")
        if previous_selection in overview_df.index:
            selected_employee_uuid = previous_selection
